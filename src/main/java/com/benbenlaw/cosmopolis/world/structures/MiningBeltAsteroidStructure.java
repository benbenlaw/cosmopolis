...
public class MiningBeltAsteroidStructure extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final Codec<MiningBeltAsteroidStructure> CODEC = RecordCodecBuilder.<MiningBeltAsteroidStructure>mapCodec(instance ->
            instance.group(MiningBeltAsteroidStructure.settingsCodec(instance),
            ...
            ).apply(instance, MiningBeltAsteroidStructure::new)).codec();
    ...
    public MiningBeltAsteroidStructure(StructureSettings config,
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        if (!MiningBeltAsteroidStructure.extraSpawningChecks(context)) {
        ...
        int maxNumber = 150;
        int startY = rand.nextInt(maxNumber) + 1;;
    ...
    public StructureType<?> type() {
        return ModStructures.MINING_BELT_ASTEROID.get(); // Helps the game know how to turn this structure back to json to save to chunks
    }
}
